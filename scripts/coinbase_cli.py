...
def run() -> int:
    ...
    try:
        if args.cmd == "accounts":
            output = {
                "summary": "Account endpoint requires Advanced Trade JWT auth; configure existing private connector for balances.",
                "data": {"auth_present": bool(os.environ.get('COINBASE_API_KEY') and os.environ.get('COINBASE_API_SECRET'))},
            }
        elif args.cmd == "list_markets":
            output = list_markets(connector, args.quote)
        elif args.cmd == "top_movers":
            output = top_movers(connector, args.window, args.quote, args.limit)
        elif args.cmd == "volatility_rank":
            output = volatility_rank(connector, args.window, args.quote, args.limit)
            ...
            output = analyze_markets(connector, args.quote, args.window, args.limit)
